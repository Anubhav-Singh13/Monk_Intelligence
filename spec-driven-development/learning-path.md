...
- **Day 5** turns the spec into a one-page technical design — the architecture — before any code is planned.
- **Day 6** teaches you to run the spec and design through an agent using a named, repeatable loop.
- **Day 7** wires Days 3–6 into a pipeline so the workflow runs end-to-end without manual steps.
- **Day 8** forces you to do the whole thing for real.
...
| [5](days/day-05-architecture-component-design.md) | Architecture & Component Design | For non-trivial features a one-page technical design — components, contracts, boundaries, decisions — sits between the spec and the plan, closing the architectural degrees of freedom the spec leaves open | The spec fixes *what*; you must fix the *shape* before the agent plans the *steps*, or the agent picks the architecture for you | Days 1, 2, 4 |
| [6](days/day-06-plan-implement-verify.md) | Plan → Implement → Verify | The core agent workflow is a three-beat loop: get the agent to produce a plan first (from the spec and design), then implement, then verify against your acceptance criteria | Teaches the steering loop as a named, repeatable pattern before wiring it into a pipeline | Days 4, 5 |
| [7](days/day-07-full-pipeline.md) | The Full Pipeline | Spec → design → agent → plan-implement-verify → CI → deploy is a single template you fill once and reuse forever; the spec, not the tool, is the durable asset | Connects all prior days into the end-to-end system | Days 1–6 |
| [8](days/day-08-capstone.md) | Capstone: Ship a Feature | Write a project constitution + one feature spec + a technical design, run the plan-implement-verify loop, push through CI, deploy | Forces full synthesis; the reader earns the outcome they came for | Days 1–7 |

---

## The learning arc

**Day 1 vs. Day 8.** On Day 1 you arrive thinking the hard part is choosing the right AI tool. By Day 3 you understand that the tool barely matters — the spec is the leverage point. By Day 8 you have a repeatable system: a constitution template you fill in once, a spec template you fill per feature, a one-page technical design for anything non-trivial, a three-beat conversation pattern with the agent, and a pipeline that takes the output to production.

**The single story.** This course is about one shift: from treating a coding agent as a smart assistant you talk to, to treating it as a deterministic pipeline you feed structured inputs. A PM already knows how to write a good PRD. This course shows you that a great spec *is* the program — you are not describing what you want, you are specifying a contract that an agent executes. Every day adds one more layer of that contract until the full pipeline snaps together on Day 7 and you ship something real on Day 8.

...
```
              Single Agent
                   |
Simple ────────────┼──────────────── Complex
(one loop)         |              (many loops, branches, memory)
                   |
            Multiple Agents
```
...
```
Goal received
  → Reason: "What do I need to find out first?"
  → Act: call search_web("...")
  → Observe: read results
  → Reason: "I have X but still need Y"
  → Act: call fetch_page("...")
  → Observe: read content
  → Reason: "I have enough. Here's my answer."
  → Respond
```
...
```
Goal received
  → Draft response (ReAct or direct)
  → Reflect: "Is this complete? Is it accurate? What did I miss?"
...
Orchestrator receives goal
  ...
  → Dispatches subtask A to Worker-1 (with its own tools + loop)
  → Dispatches subtask B to Worker-2
  → Awaits results
  → Synthesises and responds
```
...
```
Goal: analyse 5 competitors
  → Spawn 5 worker instances in parallel, one per competitor
  → All run their own ReAct loops simultaneously
  → Collect all 5 results
  → Synthesise into a single output
```
...
```
Agent completes Phase 1 (research)
  → Presents findings to human
  → Human approves, corrects, or redirects
  → Agent continues to Phase 2 with human input incorporated
```
...
```
Question: "What is the revenue impact of X?"
  → Generate answer 3–5 times independently
  → Compare: do they agree? Where do they diverge?
  → Return the consensus, flagging where uncertainty exists
```
...
```
Actor agent: produces a draft plan
Critic agent: finds flaws, contradictions, missing steps
Actor agent: revises based on critique
→ Repeat for N rounds
→ Final output is the last Actor draft
```
...
```
Goal received
  → Generate N possible next actions (branches)
  → Execute each branch partially
  → Score each branch by outcome quality
  → Continue expanding the most promising branch
  → Backtrack dead ends
  → Return best-path result
```
...
```
User: "Research the competitive landscape of AI coding assistants"
          |
          ▼
┌─────────────────────────────┐
│   PLAN-AND-EXECUTE          │  ← Decompose into: market sizing,
│   Orchestrator              │    player profiles, differentiators,
...
│Market  │   │Player   │   │ Trends   │  │ Risks   │
│sizing  │   │profiles │   │ analysis │  │         │
└────────┘   └─────────┘   └──────────┘  └─────────┘
    │              │              │             │
    └──────────────┴──────────────┴─────────────┘
                         │
                         ▼
              ┌─────────────────────┐
              │     REFLECTION      │  ← "Is this complete?
              │  Critic reviews     │    Are claims grounded?
              │  full draft         │    Are sources cited?"
              └──────────┬──────────┘
                         │
                         ▼
              ┌─────────────────────┐
              │  HUMAN-IN-THE-LOOP  │  ← Present outline before
              │  checkpoint         │    writing full report
              └──────────┬──────────┘
                         │
                         ▼
                    Final Report

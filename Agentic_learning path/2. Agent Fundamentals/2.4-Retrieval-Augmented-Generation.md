...
```
User query
    ├── Keyword search (BM25)  → result set A
    └── Vector search          → result set B
              ↓
...
Vector search → 20 candidate chunks
      ↓
Re-ranker scores each chunk against the query
      ↓
Top 5 highest-scored chunks passed to the LLM
```
...
Knowledge Graph:
  ...
  [Auth Service] --is_upstream_of--> [Order Service]
  [Order Service] --feeds_into--> [Notification Service]
  [Payment Service] --documented_in--> [Doc: Payment Architecture v2]

Query: "What breaks if payment service fails?"
  → Find [Payment Service] node
  → Traverse outbound relationships
  → Collect affected nodes: Auth, Order, Notification
  → Retrieve associated documents for each
  → LLM synthesises answer from graph traversal + documents
```
...
User Question
      ↓
Agent reasons: "Do I need to look anything up?"
      ↓ (yes)
Agent formulates retrieval query
      ↓
Retrieval → chunks returned
      ↓
Agent reasons: "Is this enough to answer? What's missing?"
      ↓ (not enough — needs another angle)
Agent formulates second retrieval query
      ↓
Retrieval → more chunks
      ↓
Agent reasons: "Now I have enough."
      ↓
Agent synthesises and responds
```
...
User question
      ↓
Agent classifies: "Is this about internal policy, product docs, or live data?"
      ├── Policy → query internal policy vector store
      ├── Product → query product documentation graph
      └── Live data → call the live data API tool
```
...
```
Is your knowledge base static and well-structured?
  YES → Start with Basic RAG. Upgrade to Advanced if quality is poor.
  NO  → Define what "dynamic" means before choosing.

Are you getting poor retrieval quality with Basic RAG?
  YES → Add hybrid search + re-ranking (Advanced RAG).
  NO  → You may not need the complexity yet.

Do questions require understanding relationships between entities?
  YES → Evaluate Graph RAG. Is the graph worth building and maintaining?
  NO  → Stay with vector-based retrieval.

Does retrieval need to be iterative or route across multiple sources?
  YES → Agentic RAG.
  NO  → Pipeline RAG (simpler, faster, cheaper).

Is latency under 2 seconds critical?
  YES → Avoid Agentic RAG for now. Pipeline RAG is more predictable.
  NO  → Agentic RAG is on the table.
```

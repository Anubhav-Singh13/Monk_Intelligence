...
──────────────────────────────────────────────────
Whiteboard (now)      →    In-context (prompt window)
Notes (retrievable)   →    External / Episodic store
Project knowledge     →    Semantic / Vector store
Skills & habits       →    Procedural (fine-tuning / system prompt)
```
...
```
WRITE: agent decides to save something
  → "I found that the user's preferred format is JSON — store this"
  → writes to: key-value store, database row, append to a file

READ: agent decides to look something up
  → "What did I find out about the user's preferences earlier?"
  → queries store → retrieves stored note → injects into context
```
...
```
Agent task: "What was the outcome of last quarter's pricing experiment?"

→ Embed the query
→ Search vector store for similar entries
→ Retrieve: [experiment summary from 3 months ago]
→ Inject into context
→ Agent answers from retrieved knowledge
```
...
```
Procedural memory:
└── System prompt: "You are a billing support agent. Always verify
    account number before taking action. Tone: calm, concise."

In-context memory:
└── The current conversation thread — all messages in this session

External memory (episodic):
└── Past support tickets for this account, retrieved at session start
    "Account #8821: 3 prior contacts. Last issue: duplicate charge, resolved."

Semantic memory:
└── Billing policy documents, embedded and searchable
    Agent retrieves: "Refund policy for annual subscriptions" when asked
```

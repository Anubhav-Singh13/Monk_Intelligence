...
Reflex ←————————————————————————————————————→ Reasoning
  |              |              |              |
Thermostat    Rule-based     RL Agent      LLM Agent
              Chatbot
```
...
Full Human Control ←——————————————————→ Full Autonomy
      |           |           |           |
  Human does    Human       Agent acts,  Agent acts
  everything   approves     human        end to end
               each step    reviews
```
...
User Goal
    |
Orchestrator Agent
    ├── Research Agent  → searches the web, returns findings
    ├── Analysis Agent  → runs calculations, returns structured data
...
```
Orchestrator
├── Scout Agent    — searches for news about competitors A, B, C
├── Extractor Agent — parses each article for pricing, features, sentiment
├── Analyst Agent  — compares findings to our product's positioning
└── Writer Agent   — drafts the report in the agreed format
```

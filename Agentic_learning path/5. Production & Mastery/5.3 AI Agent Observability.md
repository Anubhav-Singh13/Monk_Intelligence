...
```
[User Message: "What's my refund status?"]
  └── [LLM Call 1: Plan the steps]           1,200ms
        └── [Tool: lookup_order(id=9921)]       340ms
        └── [Tool: get_refund_policy()]         180ms
  └── [LLM Call 2: Generate response]         980ms
        └── [Tool: send_confirmation_email()]   210ms
```

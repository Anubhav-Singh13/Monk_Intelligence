...
User Request
     │
     ▼
API Gateway ──► Queue (SQS / Service Bus)
     │                    │
     │ job_id             │ message
     ▼                    ▼
Client polls        Worker Pool
(or webhook)        (ECS / AKS)
     ▲                    │
     │                    ▼
     └──────── Result Store (DynamoDB / Cosmos DB)
```
...
S3 upload event
      │
      ▼
SQS Queue (document_jobs)
      │
      ▼
ECS Fargate Workers (auto-scaled by queue depth)
      │
      ├── Call Bedrock (Claude) for extraction
      ├── Write results to DynamoDB
      └── Notify via SNS → email / webhook
```
...
Azure API Management
        │
        ▼
AKS Pod (agent service) ──► Azure Cache for Redis (semantic cache)
        │
        ├── Azure OpenAI (GPT-4o) for reasoning
        ├── Cosmos DB (order lookup)
        └── Azure Service Bus (escalation events → human queue)
```

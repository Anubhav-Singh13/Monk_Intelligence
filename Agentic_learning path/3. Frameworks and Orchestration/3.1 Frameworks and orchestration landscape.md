...
```
Layer 4: Protocol Standards       MCP, OpenAPI tool specs
          ────────────────────────────────────────────────
Layer 3: Enterprise SDKs          Semantic Kernel, Azure AI Foundry,
                                  AWS Bedrock Agents, Spring AI
          ────────────────────────────────────────────────
Layer 2: Agent Frameworks         LangGraph, AutoGen, CrewAI,
                                  Copilot Studio*, Pydantic AI
          ────────────────────────────────────────────────
Layer 1: Workflow Automation      Zapier, n8n, Make,
...
```
Business User Needs an Agent
          |
          ↓
    Copilot Studio
    (no-code agent builder, Teams-deployed)
          |
          ↓  (when the agent needs automation)
    Power Automate
    (Microsoft 365-native flows, business-user owned)
          |
          ↓  (when IT/engineering owns the workflow)
    Logic Apps
    (Azure-native, developer-deployed, enterprise connectors)
          |
          ↓  (when custom agent reasoning is needed)
    Semantic Kernel
    (C# / Python SDK, Azure AI Foundry integration)
          |
          ↓  (when fine-grained agent control is required)
    LangGraph / AutoGen

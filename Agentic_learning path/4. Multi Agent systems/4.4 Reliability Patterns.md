...
```
┌───────────────────────────────────────────────┐
│  Constitutional / Invariance Checks (Outer)   │
│  ┌─────────────────────────────────────────┐  │
...
Attempt 5: fails → give up (or hand off to fallback)
```
...
[Secondary: GPT-4o-mini / Claude Haiku]
  ↓ fails or times out
[Tertiary: Cached response / Template response / "We're experiencing issues"]
```
...
OPEN     → Dependency is down. Calls blocked immediately. Fallback activated.
HALF-OPEN → Probe period. One test call allowed. If it succeeds: CLOSED.
...
```
Primary:   Full AI response with personalization + reasoning
  ↓ (component fails)
Secondary: Generic AI response without personalization
  ↓ (component fails)
Tertiary:  Pre-written template response with FAQ links
  ↓ (component fails)
Quaternary: "We are experiencing high demand. Please try again shortly."
```

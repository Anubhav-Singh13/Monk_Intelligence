...
```
Trace: "user asked about refund policy"
│
├── Span: retrieve_documents          [12ms]
│     input:  "refund policy query"
│     output: [doc_1, doc_2, doc_3]
│     status: success
│
├── Span: rerank_documents            [8ms]
│     input:  [doc_1, doc_2, doc_3]
│     output: [doc_2, doc_1, doc_3]   ← reranked
│     status: success
│
├── Span: generate_response           [1.2s]
│     input:  query + [doc_2, doc_1]
│     output: "Our refund policy allows..."
│     model:  gpt-4o
│     tokens: 312 input / 87 output
│     status: success
│
...
Traces collected automatically
      ↓
Sample of traces evaluated (automated + human)
      ↓
Low-scoring traces diagnosed (which span failed?)
      ↓
Fix the component (prompt, retrieval, tool call)
      ↓
Run offline eval on golden set to confirm improvement
      ↓
Deploy → Production traffic again
```

...
(Claude,           (protocol          (your database,
 Cursor,            translation)       your file system,
 your app)                             a web search API)
```
...
[Orchestrator Agent]
        ...
        |
        | 2. Sends Task: "Find hotels in Lisbon, June 12-15, budget €150/night"
        ↓
[Hotel Search Agent]
        |
        | 3. Streams status updates back (working...)
        | 4. Returns Artifact: list of matching hotels
        ↓
[Orchestrator Agent]
        |
        | 5. Passes artifact to next agent in the workflow
```
...
[ResearchOrchestrator]
  ↓ A2A Task: "Search for 'battery technology 2025'"
[WebSearchAgent]  ←→  MCP Server: Brave Search API
  ↓ A2A Artifact: raw search results
[SummaryAgent]    ←→  MCP Server: local text processor
  ↓ A2A Artifact: condensed summaries
[ReportWriterAgent] ←→ MCP Server: file system
  ↓ A2A Artifact: formatted report
[ResearchOrchestrator]
  ↓

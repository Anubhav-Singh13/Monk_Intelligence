...
```
           ┌→ [Agent A] ─┐
User → [Orchestrator] ─→ [Agent B] ─→ [Aggregator] → Result
           └→ [Agent C] ─┘
```
...
```
[Supervisor]
   ├── [Orchestrator A: Engineering]
   │       ├── [Worker: Backend Dev]
   │       └── [Worker: QA]
   └── [Orchestrator B: Marketing]
           ├── [Worker: Copywriter]
           └── [Worker: SEO Analyst]
```
...
```
[Generator] → draft → [Critic] → feedback → [Generator] → final
```
...
```
[Task Queue] → broadcast → [Agent A bids: 0.8]
                          → [Agent B bids: 0.6]
                          → [Agent C bids: 0.9] ← wins
```

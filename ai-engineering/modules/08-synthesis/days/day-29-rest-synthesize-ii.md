# Day 29 — Rest & Synthesize II

> **Today:** No new concepts. Only consolidation of the **control, operations, graph & ontology arc** (Days 19–28).
> **Format:** Re-state → Re-derive → Full run → Quiz
> **Reading time:** ~35 min (longer if gaps surface — that's the point)
> **Prereqs:** [Days 19–28](../../../README.md)

The back half of the course turned a working agent into a *production* one — controlled, resilient, measured — then gave you graph tools for when flat structures break down and the ontology to keep meaning consistent. Today you verify it holds in memory before the capstone assembles everything.

Close all previous pages. Work from memory first. A blank is a diagnostic, not a failure.

---

## Step 1 — Re-state the 8 back-half ideas (5 min)
...
|---|---|
| Control-flow graphs (Day 26) | |
| The evaluation harness (Day 23) | |
| Multi-agent orchestration (Day 21) | |
| Context graphs (Day 25) | |
| Loop control & stopping (Day 19) | |
| Ontology & constraints (Days 27–28) | |
| Observability & cost (Day 24) | |
| Failure & recovery (Day 22) | |

<details><summary>Compare to these</summary>

1. **Control-flow graphs (Day 26):** when a loop's branching gets complex, make control explicit as a graph of nodes (steps) and conditional edges (transitions), gaining inspectability, checkpoint/resume durability, and composability.
2. **Evaluation harness (Day 23):** you can't harden what you can't measure — run the agent over a frozen task set with automatic graders (trajectory + outcome), producing comparable metrics instead of vibes.
3. **Multi-agent orchestration (Day 21):** split one loop into several with delegated, isolated context only when context demands it — and usually you shouldn't, because coordination adds cost and failure modes.
4. **Context graphs (Day 25):** store memory as a temporal graph of entities/relations so recall can traverse connections (multi-hop) and respect time (supersession), fixing flat retrieval's blind spots.
5. **Loop control & stopping (Day 19):** a loop that can't decide it's done is a bug — enforce budgets, detect stuck/no-progress, and *verify* completion claims before accepting them, all in the harness.
6. **Ontology & constraints (Days 27–28):** define your domain's classes/relations (scoped by competency questions) as the shared vocabulary extraction and queries obey, then enforce it with executable constraints at the write boundary — rejecting structural violations and routing business-rule violations, so meaning is defined *and* guaranteed.
7. **Observability & cost (Day 24):** instrument first (spans/traces of each turn), then optimize — token budgets, caching, model routing — because you can't cut what you can't see.
8. **Failure & recovery (Day 22):** treat failure as an expected input — classify transient vs. permanent, retry-with-backoff, circuit-break dead tools, make writes idempotent, and turn every error into an observation, not a crash.
</details>
...
- **[C]** the *transcript* slice (high-volume, low-density); control/accounting and durable memory are not compacted (Day 20).
- **[D]** timeout, failure classification (transient vs. permanent), backoff retry, and a circuit breaker (Day 22).
- **[E]** *real* usage from the API response (tokens/cost), not estimates.
- **[F]** the model's "done" is a *claim*, not a fact — it can be wrong/lie; verify objectively (run tests / re-check state) where possible (Day 19).
- **[G]** untrusted input — validate before executing (Day 8, carried through).
- **[H]** observability / a trace span — you instrument first, then optimize (Day 24).
</details>
...
1. Your agent has become a tangle of nested `if/break`s and you need it to pause for human approval mid-run and resume tomorrow. What structural change enables this, and why can't a plain loop do it? *(Day 26)*
2. A tool times out; your harness retries and a duplicate record appears in the database. Which two mechanisms failed, and whose *design decision* (which earlier day) is the root cause? *(Day 22)*
3. You want to cut your agent's cost by 40%. What must you do *before* changing anything, and name two levers you'd then reach for. *(Day 24)*
4. Your agent passes your eval set but a stakeholder says it "feels worse" than last week. Reconcile — and say what this reveals about eval sets. *(Day 23 × Day 21)*
5. **Cross-concept:** You're building a long-running research agent that must (a) run for hours with human check-ins, (b) remember evolving facts across sessions, and (c) not hallucinate stale conclusions. Name which *graph* solves each of (a)–(c), and explain why control-flow graphs and context graphs are *different* graphs doing *different* jobs — not the same tool. *(Day 25 × Day 26)*
...
2. **Retry** (retried a non-idempotent write) and **idempotency** (the write wasn't safe to run twice) failed. Root cause is a **Day 8** tool-design decision: idempotency is a property you build into a mutating tool at the boundary (via idempotency keys / check-then-act); it can't be bolted on in the retry layer, which can't tell a legitimate second write from a duplicate.
3. **Before:** instrument — get a trace/breakdown of where tokens and cost actually go per turn (Day 24); you can't optimize what you haven't measured, and the cost is usually concentrated somewhere surprising. **Then** levers: prompt/context **caching** (reuse a stable prefix), **model routing** (cheap model for easy turns, strong for hard), tighter **context budgets** (fewer tokens/turn via compaction), and reducing turns via better control.
...
Today closed the gap between "I read the production arc" and "I can rebuild it cold." It opens the capstone: you now hold all seven disciplines and their production hardening; tomorrow you compose them under real constraints.

---

## Connect it back

The back half gave you control, resilience, measurement, and graph structures — the difference between a demo and something you'd put in front of users. The single sharp question you carry into the capstone: *given a real task, which of the seven disciplines does it stress most, and what is the smallest system that solves it while staying controlled, measured, recoverable, and coherent in meaning?*

---

## Navigation

← **Previous:** [Day 28 — Business Rules & Constraints](../../07-ontology-engineering/days/day-28-business-rules-and-constraints.md)  
→ **Next:** [Day 30 — Capstone: Build & Harden an AI System](day-30-capstone-build-and-harden.md)

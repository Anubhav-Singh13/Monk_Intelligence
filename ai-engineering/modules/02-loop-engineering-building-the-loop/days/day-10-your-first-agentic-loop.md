# Day 10 — Your First Agentic Loop
...
> **Reading time:** ~45 min (code day) · **Prereqs:** Day 7, Day 11
> **Primary source for today:** Dex Horthy, "12-Factor Agents: Patterns of Reliable LLM Applications," AI Engineer 2025.
> **Before you start:** Recall Day 9's load-bearing idea — one sentence, no looking: *where does an agent's capability come from if the model never improves mid-run, and what four-step cycle delivers it?*

## The hook (2–4 min)

You have all four pieces now: an OS-shaped harness (Day 2), a reason to loop (Day 9), tools as requests the harness runs (Day 7), and context as the payload you assemble (Day 11). Today they snap together.

Here's the claim that surprises people: **there is no "agent framework" hiding at the center of an agent.** No secret sauce. When you strip away every abstraction, an agent is a `while` loop wrapped around an API call. That's it. If you can write `dispatch` (you did, Day 7), you can write a full agent this afternoon.

By the end of this page you'll have a real one — it'll read files, run commands, and solve a small multi-step task on its own — and you'll be able to point at every single line and say what it does. That's the whole L2 promise, delivered on Day 10. Everything after this is making *this same loop* robust, cheap, and safe.

## Building the intuition (10–15 min)

Recall Day 9's loop shape. Let's give its one iteration a name we'll use for the rest of the course: a **turn**. One turn = assemble context → call model → handle output (act or finish). The agent's whole life is turns, stacked:
...
flowchart TD
    ...
    OUT -->|tool call| RUN[dispatch tool<br/>= Day 7]
...
1. **Assemble** the context (today: naïvely append everything — Day 12 makes it smart).
2. **Call** the model (Day 2's pure function).
3. **Decide** if the output is a final answer or a tool request (Day 8 makes parsing robust).
4. **Dispatch** the tool if so (Day 7's `dispatch`).
5. **Append** the call and its result to history, and **guard** against looping forever (Day 19).

The single most important design decision is hiding in step 5's arrow back to step 1: **the history *is* the state, and you own it.** This is Dex Horthy's "own your context window" and "agents as stateless reducers": the model is stateless (Day 2), so *your loop's list of messages is the agent's entire memory.* You append to it; you'll later prune it (Day 14). Everything the agent "knows" is in that list, which *you* control. Frameworks hide this list; you're going to hold it in your hand.

Modern APIs give you a gift that makes step 3 easy: **native tool calling.** Instead of the model writing a tool request as freeform text you have to regex out, the API returns it as a *structured* object — a distinct "tool_use" block with a name and parsed arguments — and tells you the reason it stopped ("the model wants to use a tool" vs. "the model is done"). You feed the tool result back as a matching "tool_result" block. This is just Day 7's request/observation cycle, with the parsing done for you. (Day 8 covers what to do when you *don't* have this luxury, and why you still validate even when you do.)

## The formal picture (10–15 min)

Here is a complete, runnable agent. Read it twice: once for shape, once line by line. It reuses Day 7's `dispatch` idea and adds the loop.
...
# --- 1. Tools: schema (for the model) + implementation (for the harness), Day 7 ---
...
def dispatch(name: str, args: dict) -> str:            # Day 7, unchanged
    tool = TOOLS.get(name)
    if tool is None:
...
def run_agent(task: str, max_turns: int = 10) -> str:
    ...
    for turn in range(max_turns):                     # runaway guard, Day 19
        # (a) assemble context — today: just send the whole history (Day 12 upgrades this)
        ...
        for block in response.content:
            if block.type == "tool_use":
                observation = dispatch(block.name, block.input)   # Day 7
                tool_results.append({
                    "type": "tool_result",
    ...
    return "Stopped: hit max_turns without finishing."   # budget exhausted, Day 19
...
|---|---|---|
| `TOOLS`, `dispatch` | tools = schema + fn + binding | Day 7 |
| `history = [...]` | the state you own; the agent's memory | Day 2, Day 11 |
| `for turn in range(max_turns)` | the loop + runaway guard | Day 9, Day 19 |
| `messages=history` | context assembly (naïve today) | Day 11, Day 12 |
| `client.messages.create` | the stateless model call | Day 2 |
| `stop_reason != "tool_use"` | decide: done vs. act | Day 8 |
| the `tool_use` / `tool_result` dance | request → execute → observe | Day 9, Day 7 |
...
- **Context assembly is one line today, and it's a time bomb.** `messages=history` sends *everything*, forever. By Day 11's logic this overflows and rots around turn 20–30. That single naïve line is why Days 12, 13, 16, 14, 20 exist. We ship it naïve today on purpose — you can't improve what you haven't felt break.
- **Termination is half-built.** We stop on "model says done" or "out of turns." That's the floor. Day 19 adds the rest (detecting stuck loops, budgets in tokens/dollars/time, done-verification).

## Where it breaks / what it is not (3–5 min)

- **This is not production-ready, and that's fine.** No retries (Day 22), no real context management (Days 12–14), no eval (Day 23), no tracing (Day 24), permissive tools (Day 8). It's the *correct skeleton*; the rest of the course is muscle.
- **Native tool calling isn't magic parsing insurance.** The API parses the *shape* for you, but the *values* are still untrusted model output (Day 7). `block.input` can contain a malicious path. Structured ≠ validated. Day 8.
- **"It worked on my task" ≠ "it works."** A loop that solves one task can loop forever or thrash on the next. You have no idea how reliable this is until you *measure* it (Day 23). Resist trusting the demo.
...
```
Three turns, then it's done — you can *see* the observe→decide→act rhythm from Day 9, now automated. On the unfinishable task you'll see `turn 0..9` all `tool_use`, then `"Stopped: hit max_turns"`. That clean stop is your Day 19 floor working.</details>
...
<details><summary>Worked answer</summary>`len(str(history))` grows monotonically because you append every model output and every tool result forever — classic Day 9 accumulator behavior. It becomes a problem as it approaches the model's context limit `L` (hard failure/truncation) and, well before that, when old content shoves the current goal into the low-attention **middle** (Day 11's Lost-in-the-Middle → drift/rot). The *first* thing to drop is **old, superseded tool observations** — e.g. the full text of a file you read 10 turns ago and already extracted what you needed from — because they're bulky, stale, and reproducible on demand. You do **not** drop the **system prompt** (it holds the standing instructions + tool menu, and sits in the strong start position) or the **latest observation** (it's the freshest evidence, in the strong end position, and the whole reason the last action happened). This selective, placement-aware pruning is exactly **context assembly (Day 12)** and **compaction (Day 14)**.</details>
...
← **Previous:** [Day 9 — Why You Need a Loop](day-09-why-you-need-a-loop.md)  
→ **Next:** [Day 11 — Context Is Everything It Sees](../../03-context-engineering/days/day-11-context-is-everything.md)

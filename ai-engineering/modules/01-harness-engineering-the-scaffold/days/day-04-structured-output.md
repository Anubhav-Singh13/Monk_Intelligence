# Day 4 — Structured Output & the Parse Boundary
...
> **Reading time:** ~35 min (code day) · **Prereqs:** Day 3
...
> **Before you start:** Recall Day 3's load-bearing idea — one sentence, no looking: *what makes a prompt a "program" rather than a wish, and what's the single highest-leverage habit for predictable output?*
...
Recall Day 3: a prompt narrows the output distribution but never fully pins it — the model *usually* obeys the format. "Usually" is fine for a human reader and fatal for a parser. Between the model and your code you need a checkpoint, exactly like the boundary between a web form and your database:
...
def classify(ticket: str, max_retries: int = 2) -> Classification:
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            # failure becomes feedback, not a crash (Day 6 / Day 20 reflex)
...
- **Retry-with-feedback is the standard repair loop.** On a parse/validation failure, append the bad output and the error, and ask again. This works because the model is good at *correcting* a named mistake — a mini version of the recovery you'll build on Day 20. Cap the retries (probabilistic ≠ guaranteed to ever succeed).
- **This is the direct ancestor of tool calling.** A tool call (Day 5) *is* structured output: the model emits `{"name": "read_file", "input": {"path": "..."}}` conforming to the tool's schema, and your harness parses+validates it before dispatch. Everything you learned today — schema-constrain, then validate, treat as untrusted — is exactly the tool boundary tomorrow. Native "tool calling" is just schema-constrained structured output with a dispatch step bolted on.
- **Structured output is where prompting meets systems.** Day 3 made output *predictable*; today makes it *trustworthy to code*. The step up is from "a human can read it" to "a program can depend on it" — the threshold an LLM call must cross to be a real system component.
...
- **Retries cost tokens and latency.** Each repair round is another call. If you're retrying often, the fix is usually a better schema or clearer prompt, not more retries — measure your parse-failure rate (Day 22) and fix the cause.
...
← **Previous:** [Day 3 — Prompting as Programming](day-03-prompting-as-programming.md)  
→ **Next:** [Day 5 — Tools: The Model's Hands](day-05-tools-the-models-hands.md)

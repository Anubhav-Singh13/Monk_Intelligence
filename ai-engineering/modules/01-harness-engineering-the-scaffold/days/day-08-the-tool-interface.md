...
> **Reading time:** ~40 min (code day) · **Prereqs:** Day 7, Day 6
> **Primary source for today:** Anthropic, "Writing Effective Tools for AI Agents," 2025.
> **Before you start:** Recall Day 7's load-bearing idea — one sentence, no looking: *what are the three parts of a tool, and who executes the real function — the model or the harness?*

## The hook (2–4 min)

The `read_file` tool you built on Day 7 is loose. Picture a harness using it: a user asks it to "summarize the project," and the model, reasoning helpfully, emits:
...
The lesson isn't "the model is dangerous." It's that **the gap between the model and your code is a security boundary, and Day 7 left it wide open.** Today we install the guard rail — and, just as importantly, we learn that a well-*described* tool is what makes the model emit good calls in the first place.

## Building the intuition (10–15 min)

Recall Day 7's stance: *model output is untrusted input.* On Day 7 we relied on native tool calling to parse the *shape* of a call, and quietly assumed the *values* were fine. They're not. The API guarantees you a well-formed object; it guarantees nothing about whether `path` points somewhere it should.
...
The yellow **validate** box is what Day 7 was missing. Note where every path ends: **back to the model as an observation.** This is the second key intuition — *errors are not exceptions to crash on; they're observations to feed back.* A rejected call, a bad path, a tool that threw — each becomes a text result the model reads and can recover from ("that path is outside the workspace; let me try a relative one"). Your harness should almost never crash on a tool call. It should turn every outcome — success, rejection, or error — into an observation and keep the loop alive. (This graceful-degradation instinct is the seed of Day 22.)

## The formal picture (10–15 min)

A production tool has **five** parts (Day 7's three, plus two):
...
Here is Day 7's `dispatch`, hardened:
...
- **Total function over outcomes.** `dispatch` returns a string for *every* input: unknown tool, missing arg, extra arg, thrown exception, success. There is no input that crashes it. This totality is what keeps the loop robust (Day 22 formalizes it as fault isolation).
...
← **Previous:** [Day 7 — Tools: The Model's Hands](day-07-tools-the-models-hands.md)  
→ **Next:** [Day 9 — Why You Need a Loop](../../02-loop-engineering-building-the-loop/days/day-09-why-you-need-a-loop.md)

...
> **Reading time:** ~40 min (code day) · **Prereqs:** Day 8, Day 17
> **Primary source for today:** Hamel Husain, "Your AI Product Needs Evals," 2024.
> **Before you start:** Recall Day 20's load-bearing idea — one sentence, no looking: *transient vs. permanent failure — the right response to each, and the four Nygard stability patterns?*

## The hook (2–4 min)

You spent Days 10–20 making changes: budgeted context, memory, compaction, stuck-detection, retries. Now answer honestly: **did any of them actually make your agent better?**
...
Day 17 gave you *done-verification*: an objective check that one run finished correctly (run the tests, green/red). An **eval** is that idea generalized to a *set* of tasks, run *repeatably*, producing a *score*. Done-verification asks "did this run succeed?"; eval asks "across 50 representative tasks, what fraction succeed, and did my change move that fraction?"
...
def grade_fix_test(final_state, trajectory) -> dict:
    ok, _ = run_pytest(final_state["workspace"])          # outcome: objective (Day 17 verifier)
    return {
        "outcome": ok,
...
- **Eval is done-verification (Day 17) lifted from one run to a population.** The same objective checks you built to verify a single completion become the graders for a whole task set. If you built strong verifiers on Day 17, your eval is half-built; if you couldn't build a verifier for a task, that task is hard to eval — an important signal about the task itself.
...
- **Eval isn't only for shipping — it's for *developing*.** Its highest value is the tight loop: change compaction → run eval → see success go 62%→68% (or 62%→55%, catch the regression). Without it, every change on Days 10–20 was a guess.
...
Days 10–20 were changes you *hoped* helped; today you built the instrument that tells you the truth — a repeatable eval scoring whole trajectories across a fixed task set, reporting capability (success) *and* reliability (pass^k) *and* efficiency, generalizing Day 17's done-verification from one run to a population ([the numbers that turn every prior day's knob into a measured trade](../../04-loop-engineering-control-and-orchestration/days/day-17-loop-control-and-stopping.md)). The eval *produces* metrics — turns, tokens, cost, latency — but doesn't yet *explain* them or let you cut them. Tomorrow: **observability & cost** — instrument first, then optimize. The question you can now answer: *you changed your compaction threshold and the agent "feels" better — why is that sentence a red flag, and what replaces it?*
...
← **Previous:** [Day 20 — Failure & Recovery](day-20-failure-and-recovery.md)  
→ **Next:** [Day 22 — Observability & Cost](day-22-observability-and-cost.md)

...
> **Reading time:** ~40 min (code day) · **Prereqs:** Day 10, Day 19
> **Primary source for today:** Hamel Husain, "Your AI Product Needs Evals," 2024.
> **Before you start:** Recall Day 22's load-bearing idea — one sentence, no looking: *transient vs. permanent failure — the right response to each, and the four Nygard stability patterns?*

## The hook (2–4 min)

You spent Days 12–22 making changes: budgeted context, memory, compaction, stuck-detection, retries. Now answer honestly: **did any of them actually make your agent better?**
...
Day 19 gave you *done-verification*: an objective check that one run finished correctly (run the tests, green/red). An **eval** is that idea generalized to a *set* of tasks, run *repeatably*, producing a *score*. Done-verification asks "did this run succeed?"; eval asks "across 50 representative tasks, what fraction succeed, and did my change move that fraction?"
...
- **τ-bench** (tool-agent-user): the grader compares the *final database state* to a goal state, and introduces **pass^k** — run each task *k* times and measure how often it passes *all k*. Because agents are stochastic (Day 2: temperature), a task that passes once but fails on retry isn't reliably solved. Reliability is a distribution, not a single run.
...
class Task:
    id: str
    prompt: str
    setup: callable          # prepare a fresh sandbox/state (Day 8 workspace)
...
def grade_fix_test(final_state, trajectory) -> dict:
    ok, _ = run_pytest(final_state["workspace"])          # outcome: objective (Day 19 verifier)
    return {
        "outcome": ok,
...
- **Eval isn't only for shipping — it's for *developing*.** Its highest value is the tight loop: change compaction → run eval → see success go 62%→68% (or 62%→55%, catch the regression). Without it, every change on Days 12–22 was a guess.
...
Days 12–22 were changes you *hoped* helped; today you built the instrument that tells you the truth — a repeatable eval scoring whole trajectories across a fixed task set, reporting capability (success) *and* reliability (pass^k) *and* efficiency, generalizing Day 19's done-verification from one run to a population ([the numbers that turn every prior day's knob into a measured trade](../../04-loop-engineering-control-and-orchestration/days/../../04-loop-engineering-control-and-orchestration/days/day-19-loop-control-and-stopping.md)). The eval *produces* metrics — turns, tokens, cost, latency — but doesn't yet *explain* them or let you cut them. Tomorrow: **observability & cost** — instrument first, then optimize. The question you can now answer: *you changed your compaction threshold and the agent "feels" better — why is that sentence a red flag, and what replaces it?*
...
← **Previous:** [Day 22 — Failure & Recovery](day-22-failure-and-recovery.md)  
→ **Next:** [Day 24 — Observability & Cost](day-24-observability-and-cost.md)

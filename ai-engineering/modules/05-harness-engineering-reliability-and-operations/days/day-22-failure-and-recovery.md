...
> **Reading time:** ~40 min (code day) · **Prereqs:** Day 8, Day 19
> **Primary source for today:** Michael T. Nygard, *Release It!*, 2nd ed., Pragmatic Bookshelf, 2018 (Stability Patterns).
> **Before you start:** Recall Day 21's load-bearing idea — one sentence, no looking: *when should you split one loop into several agents, and what is the main cost of doing so?*
...
- **Failure handling has layers, and they compose with Day 19.** Per-call: timeout + retry + breaker (this page). Per-loop: budgets + stuck-detection (Day 19). A retried-out tool becomes an error observation; enough error observations may trip stuck-detection or exhaust the budget → escalation. The controls nest: the call layer absorbs blips, the loop layer bounds the rest.
- **Retries interact with budgets — count them.** Three retries with backoff can add seconds and tokens (if you re-call the model). Your Day 19 budget (time, cost) must account for retry overhead, or a flaky environment silently blows through it. Retry is not free; it trades latency for success probability.
- **Idempotency is a *tool design* property, decided at the boundary (Day 8).** You can't bolt safety onto a naturally-unsafe write after the fact. Mutating tools should be designed with idempotency keys or check-then-act from the start — which is why write/execute tools deserved extra scrutiny on Day 8. Read tools need none of this; the danger scales with side effects.
- **Recovery is harness *and* model.** The harness handles *mechanical* recovery (retry the blip, trip the breaker). The *model* handles *semantic* recovery — adapting strategy when it sees an error observation ("auth denied → I need different credentials → ask the user"). Shinn et al.'s **Reflexion** is exactly this model-side loop: reflect on the failure in language, change approach. The two layers cover different failures: mechanical for transient infrastructure, semantic for "my plan was wrong." Give the model *informative* error observations (Day 8) so its semantic recovery has something to work with.
- **Fail toward safety and honesty.** When recovery is exhausted, the terminal state is *not* a crash and *not* a silent wrong answer — it's an honest, escalated failure with context (Day 19's Blocked/give-up policy): "Couldn't complete: the deploy API has been unavailable for 30s after 3 attempts. Here's what I did finish." A bounded, truthful failure is a *successful* production outcome.
...
- **Circuit breakers can mask real problems.** A breaker that trips and fails fast keeps your agent alive but *hides* that a dependency is down. Pair breakers with observability (Day 24) so a tripped breaker *alerts* rather than silently degrading. Resilience without visibility is just well-hidden failure.
...
The bottleneck arc made your agent *smart* about context and *honest* about stopping; today made it *survive contact with production* — timeouts, retries, breakers, and idempotency turn "any failure crashes the run" into "failure is a bounded, recoverable observation," splitting recovery into mechanical (harness) and semantic (model, à la Reflexion) ([the Day 8 "every outcome is an observation" principle](../../01-harness-engineering-the-scaffold/days/../../01-harness-engineering-the-scaffold/days/day-08-the-tool-interface.md) scaled to infrastructure, nested inside [Day 19's controls](../../04-loop-engineering-control-and-orchestration/days/../../04-loop-engineering-control-and-orchestration/days/day-19-loop-control-and-stopping.md)). But you've been guessing at whether all this actually makes the agent *better* — you have no numbers. Tomorrow: **the evaluation harness** — how you measure, so you can stop guessing. The question you can now answer: *your agent retries a timed-out tool and creates a duplicate database record — which two patterns failed, and whose design decision was the root cause?*
...
← **Previous:** [Day 21 — Multi-Agent Orchestration](../../04-loop-engineering-control-and-orchestration/days/day-21-multi-agent-orchestration.md)  
→ **Next:** [Day 23 — The Evaluation Harness](day-23-the-evaluation-harness.md)

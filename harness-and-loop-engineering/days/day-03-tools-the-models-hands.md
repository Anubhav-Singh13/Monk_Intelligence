...
> **Before you start:** Recall Day 2's load-bearing idea — one sentence, no looking: *Where does an agent's capability come from if the model itself never gets smarter during a run, and what four-step cycle delivers it?*
...
- **The model can request a tool that doesn't exist, or with wrong args.** It hallucinates. `{"tool": "send_emial", ...}` (typo) or a missing required field will happen. The harness needs a graceful answer ("no such tool" / "invalid args") fed back as an observation — not a crash. (Day 6, Day 14.)
...
Notice: exactly one line (`tool["fn"](**args)`) touches the world, and it's guarded on all sides. The model can throw anything at `dispatch`; the harness stays standing. This function reappears — hardened — on Day 6, and drops straight into your loop on Day 5.</details>
...
<details><summary>Worked answer</summary>Add a **path-confinement check** inside `read_file` (or, better, in a validation step *before* dispatch calls the function): resolve the requested path to an absolute path and reject it if it escapes an allowed root. e.g. `p = (ROOT / path).resolve(); if not str(p).startswith(str(ROOT.resolve())): return "Error: path outside workspace"`. The key point: this belongs at the **tool boundary in the harness**, never in the model — you cannot trust the model to police its own requests. This is the heart of **Day 6 (the tool interface)** and connects to **Day 14 (failure/safety)**. Exposing a tool without confining its inputs is how a helpful `read_file` becomes a data-exfiltration primitive.</details>
...
Yesterday the loop gave the model a way to take steps; today tools gave those steps *teeth* — a way to reach the world through requests the harness fulfills ([recall Day 2's Act/Observe steps](day-02-why-you-need-a-loop.md)). But notice we've been hand-waving one thing on both days: the model's output is *text*, and we keep pretending we can cleanly read a tool request out of it. That's a lie of convenience. Tomorrow we confront the other half of the harness's job — not the model's hands, but its **eyes and memory**: *context* — everything the model sees each turn, which is the seam where your hardest problems live. The question you can now answer: *when an agent "uses a tool," what did the model actually do, and what did the harness do?*
...
← **Previous:** [Day 2 — Why You Need a Loop](day-02-why-you-need-a-loop.md)  
→ **Next:** [Day 4 — Context Is Everything It Sees](day-04-context-is-everything.md)

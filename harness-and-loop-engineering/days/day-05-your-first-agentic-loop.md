...
> **Reading time:** ~45 min (code day) · **Prereqs:** Day 3, Day 4
> **Primary source for today:** Dex Horthy, "12-Factor Agents: Patterns of Reliable LLM Applications," AI Engineer 2025.
> **Before you start:** Recall Day 4's load-bearing idea — one sentence, no looking: *What is the model's "entire mind" on a given turn, and why can a fact that's present in context still be effectively invisible?*

## The hook (2–4 min)

You have all four pieces now: an OS-shaped harness (Day 1), a reason to loop (Day 2), tools as requests the harness runs (Day 3), and context as the payload you assemble (Day 4). Today they snap together.

Here's the claim that surprises people: **there is no "agent framework" hiding at the center of an agent.** No secret sauce. When you strip away every abstraction, an agent is a `while` loop wrapped around an API call. That's it. If you can write `dispatch` (you did, Day 3), you can write a full agent this afternoon.

By the end of this page you'll have a real one — it'll read files, run commands, and solve a small multi-step task on its own — and you'll be able to point at every single line and say what it does. That's the whole L2 promise, delivered on Day 5. Everything after this is making *this same loop* robust, cheap, and safe.

## Building the intuition (10–15 min)

Recall Day 2's loop shape. Let's give its one iteration a name we'll use for the rest of the course: a **turn**. One turn = assemble context → call model → handle output (act or finish). The agent's whole life is turns, stacked:
...
flowchart TD
    ...
    OUT -->|tool call| RUN[dispatch tool<br/>= Day 3]
...
# --- 1. Tools: schema (for the model) + implementation (for the harness), Day 3 ---
...
def dispatch(name: str, args: dict) -> str:            # Day 3, unchanged
    tool = TOOLS.get(name)
    if tool is None:
...
def run_agent(task: str, max_turns: int = 10) -> str:
    ...
    for turn in range(max_turns):                     # runaway guard, Day 12
        # (a) assemble context — today: just send the whole history (Day 7 upgrades this)
        ...
        for block in response.content:
            if block.type == "tool_use":
                observation = dispatch(block.name, block.input)   # Day 3
                tool_results.append({
                    "type": "tool_result",
    ...
    return "Stopped: hit max_turns without finishing."   # budget exhausted, Day 12
...
|---|---|---|
| `TOOLS`, `dispatch` | tools = schema + fn + binding | Day 3 |
| `history = [...]` | the state you own; the agent's memory | Day 1, Day 4 |
| `for turn in range(max_turns)` | the loop + runaway guard | Day 2, Day 12 |
| `messages=history` | context assembly (naïve today) | Day 4, Day 7 |
| `client.messages.create` | the stateless model call | Day 1 |
| `stop_reason != "tool_use"` | decide: done vs. act | Day 6 |
| the `tool_use` / `tool_result` dance | request → execute → observe | Day 2, Day 3 |
...
- **Context assembly is one line today, and it's a time bomb.** `messages=history` sends *everything*, forever. By Day 4's logic this overflows and rots around turn 20–30. That single naïve line is why Days 7, 8, 10, 11, 13 exist. We ship it naïve today on purpose — you can't improve what you haven't felt break.
- **Termination is half-built.** We stop on "model says done" or "out of turns." That's the floor. Day 12 adds the rest (detecting stuck loops, budgets in tokens/dollars/time, done-verification).

## Where it breaks / what it is not (3–5 min)

- **This is not production-ready, and that's fine.** No retries (Day 14), no real context management (Days 7–11), no eval (Day 15), no tracing (Day 16), permissive tools (Day 6). It's the *correct skeleton*; the rest of the course is muscle.
- **Native tool calling isn't magic parsing insurance.** The API parses the *shape* for you, but the *values* are still untrusted model output (Day 3). `block.input` can contain a malicious path. Structured ≠ validated. Day 6.
- **"It worked on my task" ≠ "it works."** A loop that solves one task can loop forever or thrash on the next. You have no idea how reliable this is until you *measure* it (Day 15). Resist trusting the demo.
...
```
Three turns, then it's done — you can *see* the observe→decide→act rhythm from Day 2, now automated. On the unfinishable task you'll see `turn 0..9` all `tool_use`, then `"Stopped: hit max_turns"`. That clean stop is your Day 12 floor working.</details>
...
<details><summary>Worked answer</summary>`len(str(history))` grows monotonically because you append every model output and every tool result forever — classic Day 2 accumulator behavior. It becomes a problem as it approaches the model's context limit `L` (hard failure/truncation) and, well before that, when old content shoves the current goal into the low-attention **middle** (Day 4's Lost-in-the-Middle → drift/rot). The *first* thing to drop is **old, superseded tool observations** — e.g. the full text of a file you read 10 turns ago and already extracted what you needed from — because they're bulky, stale, and reproducible on demand. You do **not** drop the **system prompt** (it holds the standing instructions + tool menu, and sits in the strong start position) or the **latest observation** (it's the freshest evidence, in the strong end position, and the whole reason the last action happened). This selective, placement-aware pruning is exactly **context assembly (Day 7)** and **compaction (Day 11)**.</details>
...
The intuition arc (Days 1–4) becomes a working machine today: you built an agent that loops, reasons, uses tools, and stops — and you own every line, especially the `history` list that *is* its mind ([the payoff of Day 4's "context is everything"](day-04-context-is-everything.md)). But you also planted the course's central problem in one innocent line: `messages=history` sends everything forever. Tomorrow we harden the *tool boundary* — the seam where model output meets your code — because before we make context smart, we make actions safe. The question you can now answer: *what is the smallest complete agent, and which single line in it guarantees you'll need the rest of this course?*
...
← **Previous:** [Day 4 — Context Is Everything It Sees](day-04-context-is-everything.md)  

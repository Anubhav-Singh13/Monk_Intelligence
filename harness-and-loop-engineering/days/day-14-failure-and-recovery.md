...
> **Reading time:** ~40 min (code day) · **Prereqs:** Day 6, Day 12
> **Primary source for today:** Michael T. Nygard, *Release It!*, 2nd ed., Pragmatic Bookshelf, 2018 (Stability Patterns).
> **Before you start:** Recall Day 12's load-bearing idea — one sentence, no looking: *What are the five ways a turn can end, and which three harness controls catch the non-success modes?*
...
- **Failure handling has layers, and they compose with Day 12.** Per-call: timeout + retry + breaker (this page). Per-loop: budgets + stuck-detection (Day 12). A retried-out tool becomes an error observation; enough error observations may trip stuck-detection or exhaust the budget → escalation. The controls nest: the call layer absorbs blips, the loop layer bounds the rest.
- **Retries interact with budgets — count them.** Three retries with backoff can add seconds and tokens (if you re-call the model). Your Day 12 budget (time, cost) must account for retry overhead, or a flaky environment silently blows through it. Retry is not free; it trades latency for success probability.
...
- **Fail toward safety and honesty.** When recovery is exhausted, the terminal state is *not* a crash and *not* a silent wrong answer — it's an honest, escalated failure with context (Day 12's Blocked/give-up policy): "Couldn't complete: the deploy API has been unavailable for 30s after 3 attempts. Here's what I did finish." A bounded, truthful failure is a *successful* production outcome.
...
- **Circuit breakers can mask real problems.** A breaker that trips and fails fast keeps your agent alive but *hides* that a dependency is down. Pair breakers with observability (Day 16) so a tripped breaker *alerts* rather than silently degrading. Resilience without visibility is just well-hidden failure.
...
← **Previous:** [Day 13 — Drill II: The Turn, End-to-End](day-13-drill-the-turn-end-to-end.md)  
→ **Next:** [Day 15 — The Evaluation Harness](day-15-the-evaluation-harness.md)

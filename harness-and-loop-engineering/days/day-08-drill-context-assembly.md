# Day 8 — Drill I: Context Assembly
...
> **Prereqs:** [Day 7](day-07-context-assembly.md), [Day 4](day-04-context-is-everything.md)
> **Before you start:** Recall Day 7's load-bearing idea — one sentence, no looking: *What is the difference between state and context, and what three jobs does `assemble_context` do in priority order?*
...
Without looking at token counts, rank these five turns by how much you'd fight to keep them in context on turn 21: `{system}`, `{task}`, turn 1 (full parser.py), turn 3 (AssertionError line 44), turn 20 (test still failing, off-by-one line 44). Justify with Day 4's positional logic.
...
### Drill 5 — Recency-only fails (callback to Day 7 stretch)
...
You call `count_tokens` on the *entire* candidate list every time you test adding a message (like Day 7's naïve loop). On a 40-turn run that's O(n²) counting calls, and each is a network round-trip. Rewrite the counting so assembly is O(n) per turn. What do you cache?

<details><summary>Answer</summary>Cache **per-message token counts** the first time you see each message, since a message's own token count never changes once created. Store `msg["tokens"]` at append time (or memoize by message id). Then assembly is a sum over cached integers — O(n) additions, **zero** network calls for already-counted messages, plus one count for the single new message this turn. The Day 7 pattern of re-counting the whole growing list each turn is the trap: it turns a cheap arithmetic problem into a quadratic pile of API calls. (This foreshadows Day 16: measure before you optimize, and token-counting is itself a cost to instrument.)</details>

### Drill 8 — Adversarial ordering (stretch, combines Day 4 + Day 7)
...
← **Previous:** [Day 7 — Context Assembly](day-07-context-assembly.md)  
→ **Next:** [Day 9 — Rest & Synthesize I](day-09-rest-synthesize-i.md)
